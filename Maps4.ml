...
let () =
  if Array.length Sys.argv < 3 then
    ...
    let type_carte = Sys.argv.(1) in
    let name = Sys.argv.(2) in

    let directory =
      if type_carte = "city" then
        "city_maps"
      else if type_carte = "department" then
        "department_maps"
      else
        failwith "Le type doit être city ou department"
    in
    ...
    let node_list = load_node_list directory name in
    let edge_list = load_edge_list directory name in

    open_map name node_list edge_list search
